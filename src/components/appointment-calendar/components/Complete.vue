<template>
  <div class="complete">
    <div class="complete__actions"></div>
    <div class="complete__help">
      <div class="complete__help__select-client">
        <span
          >{{ $t("appointmentCalendar.client") }}:
          <strong>{{ nameClient }}</strong></span
        >
      </div>
      <div class="complete__help__work">
        <h2>{{ $t("appointmentCalendar.freeInformation") }}</h2>
        <div class="complete__help__work__day">
        ...
        <div
          class="complete__help__work__doctor"
          v-for="(nameDoctor, doctorIndex) in doctors"
          :key="doctorIndex"
        >
          <span class="complete__help__work__doctor__name-doctor">{{
            nameDoctor
          }}</span
          ><br />
          <span
          ...
          ><br />
          <span
            >{{ $t("appointmentCalendar.timeRecord") }}:
            {{ timeReception[nameDoctor].startReception }} -
            {{ timeReception[nameDoctor].endReception }}</span
          >
        </div>
        <b-button
          class="complete__add-record"
          type="is-success"
          icon-pack="fas"
          icon-left="check"
          @click.native="addRecord"
          >{{ $t("appointmentCalendar.addRecord") }}</b-button
        >
      </div>
    </div>
  </div>
...
import { TypeReception } from "@/types/appointment-calendar";
...
@Component({
  props: {
    ...
    dateRecord: Date,
    selectDate: Date,
    timeReception: Object
  }
})
export default class Complete extends Vue {
  timeReception: TypeReception | undefined;
